...
import { Radio, Play, Pause, Heart, Music2, Waves, Disc3 } from 'lucide-react';
import { usePlayer } from '../lib/PlayerContext';
import { useTheme } from '../lib/ThemeContext';
import { useState, useEffect } from 'react';

const RECENT_TRACKS = [
  { id: 1, title: 'Way Maker', artist: 'Leeland', time: '10:45' },
  { id: 2, title: 'Graça Soberana', artist: 'Lagoinha', time: '10:38' },
  { id: 3, title: 'Oceans', artist: 'Hillsong United', time: '10:30' }
];
...
export default function WebRadio() {
  ...
  const handlePlayRadio = () => {
    if (selectedVideo?.id === 'radio-1') {
      ...
      setSelectedVideo({
        id: 'radio-1',
        title: `Web Rádio ${churchName}`,
        author: churchName,
        publishedAt: new Date().toISOString(),
        type: 'music',
        thumbnail: logoUrl || '',
        src: 'https://stream.zeno.fm/8x934z3m8mzuv'
      } as any);
    }
  };
      ...
      <div className="px-6 max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 pb-32">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            {/* Abstract Background Shapes */}
             ...
             <div className="relative z-10 flex flex-col items-center text-center">
                <div className="relative w-48 h-48 lg:w-64 lg:h-64 rounded-full overflow-hidden mb-8 ios-shadow border-4 border-white/10 p-2">
                   <div className={`absolute inset-0 rounded-full bg-gradient-to-tr from-[var(--theme-color)] to-purple-600 ${isPlayingRadio ? 'animate-spin-slow' : ''} opacity-30`}></div>
                   <div className="w-full h-full rounded-full overflow-hidden bg-gradient-to-b from-neutral-900 to-black flex items-center justify-center p-6 border border-white/10">
                     <img src={logoUrl || '/church_logo_transparent.png'} alt={churchName || 'Ministério Frutos do Espírito'} className="w-full h-full object-contain filter drop-shadow-[0_0_15px_rgba(255,215,0,0.3)]" />
                   </div>
                ...
                <h2 className="text-2xl font-bold text-black dark:text-white mb-2">{churchName}</h2>
                <p className="text-black/60 dark:text-white/60 mb-8 font-medium">Louvores e Adoração 24h</p>
                ...
                <div className="w-full flex items-center gap-4 justify-between pt-6 border-t border-black/5 dark:border-white/5">
                   ...
                   <a 
                    href={`https://wa.me/?text=${encodeURIComponent(`Estou ouvindo a Rádio do ${churchName}! Junte-se a nós pelo app: ${window.location.origin}/webradio`)}`}
                    target="_blank"
                    rel="noreferrer"
          ...
          >
             ...
             <p className="text-sm text-black dark:text-white font-medium leading-relaxed">Você está ouvindo a Web Rádio oficial do {churchName}. Uma seleção de louvores 24 horas para edificar sua vida.</p>
          </motion.div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
           ...
           <div className="flex flex-col gap-3">
             {RECENT_TRACKS.map((track, idx) => (
               ...
               >
                  <div className="w-12 h-12 rounded-xl bg-[var(--theme-color)]/10 flex items-center justify-center text-[var(--theme-color)]">
           ...
           <div className="mt-4 p-6 bg-gradient-to-br from-blue-500/10 to-purple-500/10 rounded-[24px] border border-blue-500/20 text-center relative overflow-hidden">
              ...
              <h4 className="font-bold text-black dark:text-white mb-2">Apoie nossa Rádio</h4>
              <p className="text-sm text-black/60 dark:text-white/60 mb-4 font-medium leading-relaxed">Sua contribuição ajuda a manter a transmissão no ar e alcançar vidas.</p>
              <button 
                onClick={() => {
                  const el = document.getElementById('donate');
                  if (el) el.scrollIntoView({ behavior: 'smooth' });
                }}
                className="w-full py-3 rounded-xl bg-black/5 dark:bg-white/10 font-bold text-[13px] hover:bg-black/10 dark:hover:bg-white/20 transition-colors"
              >
                Contribuir
              </button>
           </div>
