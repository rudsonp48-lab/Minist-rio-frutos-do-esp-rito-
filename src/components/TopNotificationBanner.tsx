...
export default function TopNotificationBanner() {
  ...
  const isCallIncoming = activeNotification.type === 'call_incoming';
  const isCallMissed = activeNotification.type === 'call_missed';
    ...
    <div className="fixed top-3 left-0 right-0 z-[99999] flex justify-center px-3 sm:px-4 pointer-events-none">
      <AnimatePresence mode="wait">
        <motion.div
          ...
          onMouseLeave={() => {
            if (!isReplying && !isCallIncoming) resetAutoDismissTimer();
          }}
          className={`pointer-events-auto w-full max-w-lg bg-[#141419]/95 border ${isCallIncoming ? 'border-emerald-500/40 shadow-[0_20px_60px_rgba(16,185,129,0.3)]' : 'border-white/15 shadow-[0_20px_60px_rgba(0,0,0,0.85)]'} rounded-[26px] backdrop-blur-2xl overflow-hidden ring-1 ring-white/10`}
        >
          {/* Top Progress bar glow */}
          <div className={`h-1 w-full ${isCallIncoming ? 'bg-gradient-to-r from-emerald-500 via-teal-400 to-green-500 animate-pulse' : 'bg-gradient-to-r from-purple-500 via-pink-500 to-indigo-500 animate-pulse'}`} />
          ...
          <div className="p-3.5 sm:p-4">
            ...
            >
              ...
              <div className="relative shrink-0">
                ...
                <span className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full ${isCallIncoming ? 'bg-emerald-400 animate-ping' : isCallMissed ? 'bg-rose-500' : 'bg-emerald-500'} border-2 border-[#141419] shadow-sm`} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {isCallIncoming ? (
                      <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-[10px] font-bold text-emerald-300 uppercase tracking-wider flex items-center gap-1">
                        <PhoneCall className="w-3 h-3 text-emerald-400 animate-bounce" /> Chamada {activeNotification.callType === 'video' ? 'de Vídeo' : 'de Voz'}
                      </span>
                    ) : isCallMissed ? (
                      <span className="px-2 py-0.5 rounded-full bg-rose-500/20 border border-rose-500/30 text-[10px] font-bold text-rose-300 uppercase tracking-wider flex items-center gap-1">
                        <PhoneMissed className="w-3 h-3 text-rose-400" /> Chamada Perdida
                      </span>
                    ) : (
                ...
                <div className="flex items-center gap-3 mt-1.5 text-[11px] text-white/50">
                  <span>Toque para {isCallIncoming ? 'atender chamada' : 'abrir conversa'}</span>
                </div>
              </div>
            </div>

            {/* Actions for Incoming Call */}
            {isCallIncoming && (
              <div className="mt-3 pt-3 border-t border-white/10 flex items-center gap-2">
                <button
                  type="button"
                  onClick={handleBannerClick}
                  className="flex-1 py-2 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-lg active:scale-95 transition-all"
                >
                ...
                <button
                  ...
                  className="py-2 px-4 rounded-xl bg-white/10 hover:bg-rose-500/20 text-white/70 hover:text-rose-400 text-xs font-bold border border-white/10 active:scale-95 transition-all"
                >
                  Dispensar
                </button>
              </div>
            ...
            {/* Inline Quick Reply / Interactive Buttons for Chat */}
            {isChatType && !isCallIncoming && (
